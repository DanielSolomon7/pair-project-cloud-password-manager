...
class TestDelete:
    ...
    def test_function_deletes_a_secret_when_valid_name_given(
        self, secrets_manager_client
    ):
        test_client = secrets_manager_client
        test_secret_name = "name1"
    ...
    def test_function_returns_error_message_when_invalid_name_given(
        self, secrets_manager_client
    ):
        test_client = secrets_manager_client
        test_secret_name = "name1"
        assert (
            delete_secret(test_secret_name, test_client)
            == f"Secret name invalid - secret {test_secret_name} does not exist."
        )
