...
class TestRetrieve:
    ...
    def test_function_takes_a_secret_name_and_sm_client_and_returns_a_string(
    ...
    def test_function_retrieves_a_secret_and_stores_in_a_text_file(
        self, secrets_manager_client
    ):
        test_client = secrets_manager_client
        test_secret_name = "name1"
        ...
        assert (
            retrieve_secret(test_secret_name, test_client)
            == f"Secrets stored in local file: {test_secret_name}_secrets.txt"
        )

    def test_function_handles_when_a_secret_name_that_does_not_exist_is_given(
        self, secrets_manager_client
    ):
        test_client = secrets_manager_client
        test_secret_name = "name1"
        ...
        assert (
            retrieve_secret(test_secret_name, test_client)
            == f"Secret {test_secret_name} does not exist."
        )
