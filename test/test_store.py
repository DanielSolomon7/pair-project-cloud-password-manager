...
class TestStore:
    ...
    def test_function_takes_user_id_and_password__and_identifier_as_string_and_secrets_client_and_returns_a_string(self, secrets_manager_client):
        test_id = "User"
        test_password = "Password"
        test_identifier = "User Identifier"
        test_client = secrets_manager_client

        assert isinstance(store_secret(test_id, test_password, test_identifier, test_client),str)
    ...
    def test_function_creates_secret_when_valid_input_is_given(self, secrets_manager_client):
        ...
        assert test_client.get_secret_value(SecretId=test_identifier)['SecretString'] == '{"username":"User", "password":"Password"}'
